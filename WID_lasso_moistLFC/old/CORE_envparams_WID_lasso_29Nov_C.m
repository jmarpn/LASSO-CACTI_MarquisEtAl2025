...
inputs = ls('/pscratch/sd/j/jmarquis/cacti/lasso/d4_100m_5min/29Nov100m/20181129/gefs09/base/les/subset_d4/f15min/wID_3Denv_*mat')
...
%     mkdir(outdir)
    ...
    % Tv    = T.*(1+0.608.*mr_IN);  %K
    % thetav = Tv.*(100000./p_IN).^0.286; %K
    % theta = T.*(100000./p_IN).^0.286;

    % teA = T.*(100000./p_IN).^(Rd./(cpd + cl.*mr_IN));
    % teB = exp( (lv0.*mr_IN)./((cpd + mr_IN*cl).*T)   );
    % thetae = teA .* teB;

    %TdDepress = tdry_IN - TdC_IN;  %C
    ...
    for cccc = 1:length(todolist)      % cell track loop
            ...
            for y = 1:Ntimes
                for t = 1:1  %dummy var
                    ...
                    if( isnan(height)==0 & isnan(pres)==0 & isnan(tdry)==0 & isnan(mr)==0 & isnan(U)==0 & isnan(V)==0 )
                        ...
                        pCIN_mu(x,y,t)       = muCINpse;
                        pCAPEacbl_mu(x,y,t)     = mu_CAPEACBLpse;
                        pLFC_height_mu(x,y,t)   = muLFCpse;
                        pEL_height_mu(x,y,t)    = muELpse;
                        ptallenough_mu(x,y,t)   = teMUpse;

                        tallenough_sfc(x,y,t)    = teSFC;

                        pCIN_sfc(x,y,t)       = sfcCINpse;
                        pCAPEacbl_sfc(x,y,t)     = sfc_CAPEACBLpse;
                        pLFC_height_sfc(x,y,t)   = sfcLFCpse;
                        pEL_height_sfc(x,y,t)    = sfcELpse;
                        ptallenough_sfc(x,y,t)   = teSFCpse;

                        tallenough_ml(x,y,t)    = teML;

                        pCIN_ml(x,y,t)       = mlCINpse;
                        pCAPEacbl_ml(x,y,t)     = ml_CAPEACBLpse;
                        pLFC_height_ml(x,y,t)   = mlLFCpse;
                        pEL_height_ml(x,y,t)    = mlELpse;
                        ptallenough_ml(x,y,t)   = teMLpse;
                        ...
                        if(   isempty(mlLCL)==0 & isnan(mlLCL)==0   )
                            ...
                            LCL_temp_ml(x,y,t) = T(k_vari);
                        else
                            LCL_temp_ml(x,y,t) = NaN;
                        end
                        ...
                        if(   isempty(mlEL)==0 & isnan(mlEL)==0   )
                            ...
                            EL_temp_ml(x,y,t) = T(k_vari);
                        else
                            EL_temp_ml(x,y,t) = NaN;
                        end
                        ...
                        if(   isempty(mlLFC)==0 & isnan(mlLFC)==0   )
                            ...
                            LFC_temp_ml(x,y,t) = T(k_vari);
                        else
                            LFC_temp_ml(x,y,t) = NaN;
                        end
                        ...
                        PW(x,y,t) = -(100/9.8).*(1/997).*sum(chunks,'omitnan') ; %the 100 is to make it cm
                        ...
                        freezing_ht(x,y,t) = ht( frz ) ;         % m AGL
                        ...
                        %                     upslope_max(sonday,qwq) = min(u(NegU,t)); %max upsloping wind below terrain top
                        %                 elseif(isempty(NegU) == 1) %there are not easterly low-level winds
                        %                     upslope_max(sonday,qwq) = NaN ; %max upsloping wind below terrain top
                        %                 end
                        %
                        %                 upslope_mean(sonday,qwq) = LAYERMEAN(u(:,t),ht,1,kterr); %mean upsloping wind b/w AMF sfc and terrain top (neg = upslope)
                        %                 upslope_fract(sonday,qwq) = length(NegU)/kterr ;  %fraction of vertical data points between AMF surface and the terrain peak with upslope flow
                        %
                        ...
                        %                 %non-upslope flow:
                        %
                        %                 PosU = find( u(1:k5km,t) > 0 );         %jnm_new_sep8  - allowing the upslope detection to go deeper
                        %                 %PosU = find( u(1:kterr,t) > 0 );       %jnm_new_sep8
                        %                 if (isempty(PosU)==0)   %upslope stops before terrain ht
                        %                     upslope_depth(sonday,qwq) = ht(PosU(1));   %first height between AMF sfc and terrain peak at which U no longer upslopes
                        %                 elseif (isempty(PosU)==1)  % all upslope
                        %                     upslope_depth(sonday,qwq) = hterr;
                        %                 end
                        ...
                        shear_mag_bulk_0to1km(x,y,t) = ( du*du + dv*dv ).^0.5 ;

                        sdir = 270 - rad2deg( atan2(dv,du) )  ;
                        if( sdir > 360.0 )
                            sdir = sdir - 360 ;
                        end
                        shear_dir_bulk_0to1km(x,y,t) =  sdir;




                        du = U(k3km) - u10; dv = V(k3km) - v10;
                        shear_mag_bulk_0to3km(x,y,t) =  ( du*du + dv*dv ).^0.5 ;

                        sdir = 270 - rad2deg( atan2(dv,du) )  ;
                        if( sdir > 360.0 )
                            sdir = sdir - 360 ;
                        end
                        shear_dir_bulk_0to3km(x,y,t) =  sdir;




                        du = U(k6km) - u10; dv = V(k6km) - v10;
                        shear_mag_bulk_0to6km(x,y,t) =  ( du*du + dv*dv ).^0.5 ;

                        sdir = 270 - rad2deg( atan2(dv,du) )  ;
                        if( sdir > 360.0 )
                            sdir = sdir - 360 ;
                        end
                        shear_dir_bulk_0to6km(x,y,t) = sdir;



                        du = U(k9km) - u10; dv = V(k9km) - v10;
                        shear_mag_bulk_0to9km(x,y,t) =  ( du*du + dv*dv ).^0.5 ;

                        sdir = 270 - rad2deg( atan2(dv,du) )  ;
                        if( sdir > 360.0 )
                            sdir = sdir - 360 ;
                        end
                        shear_dir_bulk_0to9km(x,y,t) = sdir;
                        ...
                        rvap_925mb(x,y,t) = mr(k925mb)*1000;
                        rvap_850mb(x,y,t) = mr(k850mb)*1000;
                        rvap_700mb(x,y,t) = mr(k700mb)*1000;
                        rvap_600mb(x,y,t) = mr(k600mb)*1000;
                        rvap_500mb(x,y,t) = mr(k500mb)*1000;
                        rvap_400mb(x,y,t) = mr(k400mb)*1000;
                        rvap_300mb(x,y,t) = mr(k300mb)*1000;

                        rh_925mb(x,y,t) = rh(k925mb);
                        rh_850mb(x,y,t) = rh(k850mb);
                        rh_600mb(x,y,t) = rh(k600mb);
                        rh_700mb(x,y,t) = rh(k700mb);
                        rh_500mb(x,y,t) = rh(k500mb);
                        rh_400mb(x,y,t) = rh(k400mb);
                        rh_300mb(x,y,t) = rh(k300mb);

                        rvap_3000masl(x,y,t) = mr(k3kmasl)*1000;
                        rvap_4000masl(x,y,t) = mr(k4kmasl)*1000;
                        rvap_5000masl(x,y,t) = mr(k5kmasl)*1000;
                        rvap_6000masl(x,y,t) = mr(k6kmasl)*1000;

                        rh_3000masl(x,y,t) = rh(k3kmasl);
                        rh_4000masl(x,y,t) = rh(k4kmasl);
                        rh_5000masl(x,y,t) = rh(k5kmasl);
                        rh_6000masl(x,y,t) = rh(k6kmasl);



            		    if( isempty(muLCL)==0 & isnan(muLCL)==0 )
                            kLCLmu1500 = find( abs(ht - (muLCL+1500)) == min(abs(ht - (muLCL+1500))  ) ) ;  kLCLmu1500 = kLCLmu1500(1);   %ht index of z = 1.5km + LCL
                            rh_at_muLCLplus1500m(x,y,t) = rh(kLCLmu1500);
            		    end
                        ...
                        if( isnan(muLFCpse) == 0 )
                            ...
                            rh_at_pmuLFCplus1500m(x,y,t) = rh(kACBLmupse);

                            RH_mean_ACBL_pmu(x,y,t) =       LAYERMEAN( rh(:),    ht,kLFCmupse,kACBLmupse )  ;
                            thetae_mean_ACBL_pmu(x,y,t) =   LAYERMEAN( thetae(:),ht,kLFCmupse,kACBLmupse )  ;
                            Ucrel_mean_ACBL_pmu(x,y,t) =    LAYERMEAN( u_crel(:),ht,kLFCmupse,kACBLmupse )  ;
                            Vcrel_mean_ACBL_pmu(x,y,t) =    LAYERMEAN( v_crel(:),ht,kLFCmupse,kACBLmupse )  ;

                            du = U(kACBLmupse,t) - U(kLFCmupse,t); dv = V(kACBLmupse,t) - V(kLFCmupse,t);
                            shear_mag_bulk_ACBL_pmu(x,y,t) = ( du*du + dv*dv ).^0.5 ;

                        else

                            rh_at_pmuLFCplus1500m(x,y,t) = NaN;
                            RH_mean_ACBL_pmu(x,y,t) = NaN;
                            shear_mag_bulk_ACBL_pmu(x,y,t) = NaN;
                            thetae_mean_ACBL_pmu(x,y,t) = NaN;
                            Ucrel_mean_ACBL_pmu(x,y,t) = NaN;
                            Vcrel_mean_ACBL_pmu(x,y,t) = NaN;
                        ...
                        if(isnan(muLFC)== 1 )
                            ...
                            %
                            shear_mag_bulk_ACBL_mu(x,y,t) = NaN;
            		        ...
            		        rh_at_muLFCplus1500m(x,y,t) = NaN;
                            Ucrel_mu(x,y,t) =  NaN ;
                            Vcrel_mu(x,y,t) =  NaN ;
                            ...
                            Ucrel_mean_ACBL_mu(x,y,t)    =  NaN;
                            Vcrel_mean_ACBL_mu(x,y,t)    = NaN;
                            ...
                            thetae_mu(x,y,t) = NaN; %jnm_new_23mar2022
                            thetae_mean_subcloud_mu(x,y,t) = NaN; %jnm_new_23mar2022
                            rvap_mu(x,y,t) =  NaN;

                            thetae_mean_ACBL_mu(x,y,t) =  NaN; %jnm_new_23mar2022

                            %                         LCL_temp_mu(x,y,t) = NaN;
                            %                         LFC_temp_mu(x,y,t) = NaN;
                            %                         EL_temp_mu(x,y,t)  = NaN;

                            shear_mag_bulk_FT_mu(x,y,t) =  NaN; %jnm_new_sep8
                            shear_dir_bulk_FT_mu(x,y,t) =  NaN; %jnm_new_sep8
                        ...
                        elseif(isnan(muLFC) == 0 )  %there is an mu lfc
                            ...
                            if( isnan(cell_motion_x)==0 & isnan(cell_motion_y)==0 )
                                ...
                                Ucrel_mean_ACBL_mu(x,y,t) = LAYERMEAN( u_crel(:),ht,kLFCmu,kACBLmu);
                                Vcrel_mean_ACBL_mu(x,y,t) = LAYERMEAN( v_crel(:),ht,kLFCmu,kACBLmu);

                                %Ucrel_mean_0toACBL_mu(x,y,t) = LAYERMEAN( u_crel(:),ht,kMU,kACBLmu);
                                %Vcrel_mean_0toACBL_mu(x,y,t) = LAYERMEAN( v_crel(:),ht,kMU,kACBLmu);

                                Ucrel_mu(x,y,t) =  mean( u_crel(kMU),'omitnan');
                                Vcrel_mu(x,y,t) =  mean( v_crel(kMU),'omitnan');
                            ...
                            %ACBL humidity:
                			rh_at_muLFCplus1500m(x,y,t) = rh(kACBLmu);
                            RH_mean_ACBL(x,y,t)        =  LAYERMEAN(rh(:), ht, kLFCmu, kACBLmu);
                            ...
                            if(isnan(hMU) == 0)
                                ...
                                thetae_mean_ACBL_mu(x,y,t) =  LAYERMEAN(thetae(:), ht, kLFCmu, kACBLmu) ; %jnm_new_23mar2022
                                thetae_mean_subcloud_mu(x,y,t)  =  LAYERMEAN(thetae(:),ht,kMU,kmuLCL ) ;%jnm_new_23mar2022
                            elseif(isnan(hMU) == 1)
                            ...
                            shear_mag_bulk_ACBL_mu(x,y,t) =  ( du*du + dv*dv ).^0.5 ;
                            %
                            %             sdir = 270 - rad2deg( atan2(dv,du) )  ;
                            %             if( sdir > 360.0 )
                            %                 sdir = sdir - 360 ;
                            ...
                            shear_mag_bulk_FT_mu(x,y,t) =   ( du*du + dv*dv ).^0.5 ;                       %jnm_new_sep8

                            sdir = 270 - rad2deg( atan2(dv,du) )  ;                                             %jnm_new_sep8
                            if( sdir > 360.0 )                                                                  %jnm_new_sep8
                                sdir = sdir - 360 ;                                                             %jnm_new_sep8
                            end                                                                                 %jnm_new_sep8
                            shear_dir_bulk_FT_mu(x,y,t) = sdir;                         %jnm_new_sep8
                            ...
                            if(isnan(hMU) == 0)
                                ...
                                U_mu(x,y,t) =  U(kMU) ;
                                V_mu(x,y,t) =  V(kMU) ;
                                theta_mu(x,y,t) = theta(kMU);
                                thetav_mu(x,y,t) = thetav(kMU);
                			    thetae_mu(x,y,t) = thetae(kMU); %jnm_new_23mar2022
                                rvap_mu(x,y,t) = mr(kMU)*1000; %g/kg
                            elseif(isnan(hMU) == 1)
                                %                 U_mu = vertcat(U_mu, NaN);
                                %                 V_mu = vertcat(V_mu, NaN);
                                %                 theta_mu = vertcat(theta_mu,NaN);
                        ...
                        Vcrel_mu(x,y,t) =  NaN;
                        thetae_mu(x,y,t) =  NaN;
                        thetae_mean_subcloud_mu(x,y,t) =  NaN;
                        rvap_mu(x,y,t) =  NaN;
                        thetae_mean_ACBL_mu(x,y,t) =  NaN;
                        %                     LCL_temp_mu(x,y,t) =  NaN;
                        %                     LFC_temp_mu(x,y,t) =  NaN;
                        %                     EL_temp_mu(x,y,t) =  NaN;
                        shear_mag_bulk_FT_mu(x,y,t) =  NaN;
                        shear_dir_bulk_FT_mu(x,y,t) =  NaN;
                        shear_mag_bulk_0to1km(x,y,t) =  NaN;
                        shear_dir_bulk_0to1km(x,y,t) =  NaN;
                        shear_mag_bulk_0to3km(x,y,t) =  NaN;
                        shear_dir_bulk_0to3km(x,y,t) =  NaN;
                        shear_mag_bulk_0to6km(x,y,t) =  NaN;
                        shear_dir_bulk_0to6km(x,y,t) =  NaN;
                        rvap_850mb(x,y,t) =  NaN;
                        rvap_700mb(x,y,t) =  NaN;
                        rvap_500mb(x,y,t) =  NaN;
                        rh_850mb(x,y,t) =  NaN;
                        rh_700mb(x,y,t) =  NaN;
                        rh_500mb(x,y,t) =  NaN;

                        %enoch vars
                        rh_at_muLCLplus1500m(x,y,t) =  NaN;
                        rh_at_muLFCplus1500m(x,y,t) =  NaN;
                        ...
                        rvap_600mb(x,y,t) =  NaN;
                        rvap_400mb(x,y,t) =  NaN;
                        rvap_300mb(x,y,t) =  NaN;
                        rh_600mb(x,y,t) =  NaN;
                        rh_400mb(x,y,t) =  NaN;
                        rh_300mb(x,y,t) =  NaN;
                        rvap_3000masl(x,y,t) =  NaN;
                        rvap_4000masl(x,y,t) =  NaN;
                        rvap_5000masl(x,y,t) =  NaN;
                        rvap_6000masl(x,y,t) =  NaN;
                        rh_3000masl(x,y,t) =  NaN;
                        rh_4000masl(x,y,t) =  NaN;
                        rh_5000masl(x,y,t) =  NaN;
                        rh_6000masl(x,y,t) =  NaN;
                        ...
                        shear_mag_bulk_0to9km(x,y,t) = NaN;
                        shear_dir_bulk_0to9km(x,y,t) = NaN;

                        %                     U_4000m(x,y,t) = NaN;
                        %                     V_4000m(x,y,t) = NaN;

                        rvap_925mb(x,y,t) = NaN;
                        rh_925mb(x,y,t) = NaN;
                        rvap_600mb(x,y,t) = NaN;
                        rh_600mb(x,y,t) = NaN;
             		    ...
             		    tallenough_mu(x,y,t)    =  NaN;
             		    pCIN_mu(x,y,t)       =  NaN;
             		    pCAPEacbl_mu(x,y,t)     =  NaN;
             		    pLFC_height_mu(x,y,t)   =  NaN;
             		    pEL_height_mu(x,y,t)    =  NaN;
             		    ptallenough_mu(x,y,t)   =  NaN;


                        tallenough_sfc(x,y,t)    =  NaN;
                        pCIN_sfc(x,y,t)       =  NaN;
                        pCAPEacbl_sfc(x,y,t)     =  NaN;
                        pLFC_height_sfc(x,y,t)   =  NaN;
                        pEL_height_sfc(x,y,t)    =  NaN;
                        ptallenough_sfc(x,y,t)   =  NaN;


                        tallenough_ml(x,y,t)    =  NaN;
                        pCIN_ml(x,y,t)       =  NaN;
                        pCAPEacbl_ml(x,y,t)     =  NaN;
                        pLFC_height_ml(x,y,t)   =  NaN;
                        pEL_height_ml(x,y,t)    =  NaN;
                        ptallenough_ml(x,y,t)   =  NaN;
        ...
        if(netcdf_out == 1)
            ...
            ncfile = strcat(outdir,'WIDlasso_envparms_',splidirs(9), '_',widtime,'_', splidirs(10), '_', splidirs(11), '_', splidirs(12),'_cell',CELLID,'.nc');
            delete(ncfile);

            parcdfer_ZAwrf3(ncfile, NANdiag, CAPE_mu, LCL_height_mu ,LFC_height_mu, EL_height_mu, ...
