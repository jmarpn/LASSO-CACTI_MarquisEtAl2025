...
inputs = ls('/pscratch/sd/j/jmarquis/cacti/lasso/d4_100m_5min/22Jan100m/20190122/gefs18/base/les/subset_d4/f15min/wID_3Denv_3R_*mat')
...
for in = 1 : inlen
    ...
    disp(['splidirs(9)',splidirs(9)])
    disp(['splidirs(10)',splidirs(10)])
    disp(['splidirs(11)',splidirs(11)])
    disp(['splidirs(12)',splidirs(12)])
    disp(['splidirs(15)',splidirs(15)])
    ...
    disp([ 'widtime = ', widtime  ])
    disp(['sd9', splidirs(9)  ] )
    disp(['sd10', splidirs(10) ] )
    disp(['sd11', splidirs(11) ] )
    disp(['sd12', splidirs(12) ] )
...
%     mkdir(outdir)
    ...
    htagl_IN = WID_3Dhtagl;    % ht agl generated from model ht asl
    ...
    htagl_IN    = permute(htagl_IN,[2 4 3 1]);    
    ...
    for cccc = 1:length(todolist)      % cell track loop
        ...
        labc = strcat('start wID  ',num2str(c) );
        disp( labc )
...
%}
        ...
        for x = 1:Nlocations
		    ...
		    pres = wrf_pres(x,y,:,t);           %mb
                    ...
                    mr = permute(mr,[3 2 1]);
                    rh = permute(rh,[3 2 1]);
                    height = permute(height,[3 2 1]);      
                    htasl = permute(htasl,[3 2 1]);

                    %get rid of the sub-dirt nans with the new deepermet runs:
                    belowdirt = find( isnan(height(1:12)) ) ;
                    U(belowdirt) = [];
                    V(belowdirt) = [];
                    ht(belowdirt) = [];
                    pres(belowdirt) = [];
        ...
        if(netcdf_out == 1)
            ...
            ncfile = strcat(outdir,'WIDlasso_deeper_3R_envparms_',splidirs(9), '_',widtime,'_', splidirs(10), '_', splidirs(11), '_', splidirs(12),'.nc');
            delete(ncfile);
